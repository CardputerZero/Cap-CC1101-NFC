...
void St25r3916Driver::stopNfcA() noexcept
{
    ...
    try {
        ...
        spdlog::warn("ST25R3916 driver: failed to disable the NFC-A field: {}", exception.what());
    }
...
void St25r3916Driver::enableField(const CancellationToken& cancellation)
{
    ...
    if ((flags & kInterruptFieldCollision) != 0) {
        throw std::runtime_error("external RF field detected during NFC-A collision avoidance");
    }
    if ((flags & kInterruptFieldGuardDone) == 0) {
        throw std::runtime_error("NFC-A field-on guard timer did not complete");
    }
    ...
    if ((operation & kOperationTransmitter) == 0) {
        spdlog::error(
            "ST25R3916 driver: field-on verification failed (flags=0x{:08X}, OP_CONTROL=0x{:02X}, "
            "AUX_DISPLAY=0x{:02X})",
            flags, operation, auxiliary);
        throw std::runtime_error("RF transmitter enable was not retained after NFC-A field-on guard time");
    }
...
void St25r3916Driver::ensureNfcAField(const CancellationToken& cancellation)
{
    ...
    const auto fieldReady = [](uint8_t operation) { return (operation & expectedMask) == expectedValue; };
    ...
    const uint8_t mode     = readRegister(kRegModeDefinition);
    ...
    spdlog::warn(
        ...
        "MODE=0x{:02X}, IDENTITY=0x{:02X}); reconfiguring the field",
        operationSamples[0], operationSamples[1], operationSamples[2], mode, identity);
